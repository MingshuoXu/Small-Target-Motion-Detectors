...
def test():
    # model
    model = instancing_model('HaarSTMD', device=DEVICE)
    model.print_para()
    ...
    visualizer = FrameVisualizer(window_name=model.__class__.__name__, 
                                    ...
                                    conf_threshold=0)
    # output_path = os.path.join('C:/', 'Users', 'mings', 'Desktop', f'{model.__class__.__name__}_test_output1.mp4')
    # visualizer.setup_video_writer(output_path, fps=30, width=None, height=None)
    ...
    for color_img, gray_tensor in frame_reader:
        ...
        # result, run_time = model.process(gray_tensor)
