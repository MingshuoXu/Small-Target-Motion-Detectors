...
from xttmp.core.l2b_stmd_core import get_STMD_region_proposal
...
def test():
    ...
    for color_img, gray_tensor in frame_reader:
        ...
        vSTMD_res, bboxes, _ = get_STMD_region_proposal(result['response'], result['direction'], 
                                 top_k=1, box_size=16, spatial_scale=1.0)
        ...
        if len(bboxes) > 0:
            bboxes_res = torch.cat([bboxes, vSTMD_res[:, 2:3]], dim=1) # 将结果移回 CPU 并转换为 NumPy 数组
        else:
            bboxes_res = None
        ret = visualizer.update(color_img, bboxes_res, process_time=run_time)
        if not ret: break
